...
### Configuration - Pony Diffusion XL Optimized (Long-term Stable) ###
...
# === ONLY PONY/SDXL COMPATIBLE MODELS ===
# Removed: femboi_full_v1, femboy_otoko_no_ko, femboy (these were SD 1.5 → caused shape errors)
# All entries below are confirmed PonyXL-compatible (no more shape mismatches)
CIVITAI_MODELS_DEFAULT=(
    # Base Model - Pony Diffusion V6 XL (best for femboy/anime style)
    ...
    # PonyXL Femboy / Male Style LoRAs
    "https://civitai.com/api/download/models/324974 | $MODELS_DIR/Lora/femboysxl_v1.safetensors"                    # femboysxl_v1
    "https://civitai.com/api/download/models/2625213?type=Model&format=SafeTensor | $MODELS_DIR/Lora/male_mix_pony.safetensors"  # male_mix_pony
    "https://huggingface.co/datasets/CollectorN01/PonyXL-Lora-MyAhhArchiveCN01/resolve/main/concept/CurvyFemboyXL.safetensors | $MODELS_DIR/Lora/curvy_femboy_xl.safetensors"  # curvy_femboy_xl
)
...
install_civitai_models() {
    ...
    log "Downloading ${#models[@]} PonyXL-compatible model(s)/LoRA(s)..."
...
main() {
    ...
    log "All models are now Pony Diffusion XL compatible → no more shape mismatch errors."
}
